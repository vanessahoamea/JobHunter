...
else
{
    ...
    else
    {
        ...
        {
            http_response_code(400);
            echo json_encode(array("message" => "Fields 'institution_name', 'start_month', 'start_year' are required."));
            return;
        }
        ...
        $response = $candidate->addEducation($institutionName, $startMonth, $startYear, $endMonth, $endYear, $degree, $studyField, $description);

        if($response == 1)
        {
            http_response_code(201);
            echo json_encode(array("message" => "Added education to resume."));
        }
        else if($response == 0)
        {
